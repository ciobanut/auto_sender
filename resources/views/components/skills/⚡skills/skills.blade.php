<div class="space-y-6 max-w-3xl">
    <div class="flex items-center justify-between">
        ...
        <x-ui.button variant="default" wire:click="$set('showForm', true)">
            <x-ui.icon name="tabler.plus" class="w-4 h-4" /> {{ __('Add Skill') }}
        </x-ui.button>
    </div>
        ...
        <form wire:submit="add" class="space-y-4">
            ...
            <div class="flex items-center gap-2">
                <x-ui.button variant="default" type="submit">
                    <x-ui.icon name="tabler.check" class="w-4 h-4" /> {{ __('Add') }}
                </x-ui.button>
    ...
    <div class="bg-base-100 rounded-xl border border-base-content/5 p-12 text-center">
        <x-ui.icon name="tabler.tools" class="w-12 h-12 mx-auto text-zinc-300 dark:text-zinc-600 mb-4 empty-icon" />
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="flex items-center justify-between bg-base-100 rounded-xl border border-base-content/5 p-3 group skill-card">
            <div class="flex items-center gap-3 min-w-0">
                <x-ui.icon name="tabler.code" class="w-4 h-4 text-primary shrink-0" />
                <div class="min-w-0">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <button wire:click="remove({{ $skill->id }})" wire:confirm="{{ __('Remove this skill?') }}" class="btn-ghost btn-xs text-error opacity-0 group-hover:opacity-100 transition-opacity">
                    <x-ui.icon name="tabler.x" class="w-3.5 h-3.5" />
                </button>
            </div>
        </div>
        @endforeach
