            ...
            <header class="bg-background/80 supports-[backdrop-filter]:bg-background/60 sticky top-0 z-30 flex h-16 items-center gap-3 border-b px-4 backdrop-blur-xl lg:px-6">
                <x-ui.sheet>
                    <x-ui.sheet-trigger class="lg:hidden">
                        <x-ui.button variant="outline" size="icon" aria-label="Menu"><x-ui.icon name="tabler.menu" class="h-4 w-4" /></x-ui.button>
                    </x-ui.sheet-trigger>
                ...
                <div class="ml-auto flex items-center gap-1.5">
                    <button type="button" @click="$store.theme && $store.theme.toggle()" class="hover:bg-accent inline-flex size-9 items-center justify-center rounded-md transition-colors" aria-label="Toggle theme"><x-ui.icon name="tabler.sun" class="h-4 w-4 dark:hidden" /><x-ui.icon name="tabler.moon" class="hidden h-4 w-4 dark:block" /></button>
                    <x-ui.dropdown-menu>
                        <x-ui.dropdown-menu-trigger>
                            <button class="hover:bg-accent relative inline-flex size-9 items-center justify-center rounded-md transition-colors" aria-label="Notifications"><x-ui.icon name="tabler.bell" class="h-4 w-4" /><span class="bg-destructive absolute right-2 top-2 size-2 rounded-full"></span></button>
                        </x-ui.dropdown-menu-trigger>
                    ...
                    <x-ui.dropdown-menu>
                        <x-ui.dropdown-menu-trigger>
                            <button aria-label="Account"><x-ui.avatar class="h-8 w-8"><x-ui.avatar-fallback>AD</x-ui.avatar-fallback></x-ui.avatar></button>
                        </x-ui.dropdown-menu-trigger>
            ...
            <main class="flex-1 space-y-6 p-4 lg:p-6">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="flex gap-2">
                        <x-ui.button variant="outline" size="sm"><x-ui.icon name="tabler.calendar" class="h-4 w-4" /> Last 30 days</x-ui.button>
                        <x-ui.button size="sm"><x-ui.icon name="tabler.download" class="h-4 w-4" /> Export</x-ui.button>
                    </div>
                </div>
                                ...
                                @foreach ($orders as $o)
                                    <x-ui.table-row>
                                        ...
                                        <x-ui.table-cell>
                                            <x-ui.dropdown-menu>
                                                <x-ui.dropdown-menu-trigger>
                                                    <button class="hover:bg-accent inline-flex size-8 items-center justify-center rounded-md" aria-label="Actions"><x-ui.icon name="tabler.dots-vertical" class="h-4 w-4" /></button>
                                                </x-ui.dropdown-menu-trigger>
